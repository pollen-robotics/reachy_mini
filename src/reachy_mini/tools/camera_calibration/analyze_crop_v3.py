...
def analyze_image(image_path, aruco_dict, board):
    ...
    for i, marker_id in enumerate(marker_ids):
        ...
        marker_centers[int(marker_id[0])] = np.array([center[0]/width, center[1]/height])
...
def main():
    ...
    if len(common_ids) >= 2:
        ...
        for name, result in sorted(results.items(), key=lambda x: x[1]['resolution'][0] * x[1]['resolution'][1], reverse=True):
            ...
            print(f"{name}:")
            print(f"  Resolution: {result['resolution'][0]}x{result['resolution'][1]}")
            print(f"  Normalized distance: {distance:.4f} (reference: {ref_distance:.4f})")
            print(f"  Board occupies {scale_factor:.2%} of reference span")
            if scale_factor > 1:
                print(f"  → Board appears LARGER = MORE ZOOMED IN/CROPPED by {((scale_factor-1)*100):.1f}%")
            else:
                print(f"  → Board appears SMALLER = LESS ZOOMED IN by {((1-scale_factor)*100):.1f}%")
