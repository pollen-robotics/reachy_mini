...
| sad      | Gains 0.8 to 0.85, breath 0.9, tilt 0.8 (valence, not arousal: use   |
|          | ``energy`` for that). Slower smoothing (DIR_LERP 0.25, slow          |
|          | envelope). Head biased down 5 deg and sunk 2 mm. On some nuclei      |
|          | (15 %, at most every 2.5 s) a slow extra drop of 4 deg over 1.5 s.   |
...
class SwayRollRT:
    ...
    def __init__(
        ...
        """Create the tapper for mono float32 PCM at ``sample_rate``.

        ``emotion`` is one of :data:`EMOTIONS` (unknown names fall back to
        neutral with a warning), ``energy`` is clamped to [0, 1.5].
        """
        self._seed = int(rng_seed)
        self.sample_rate = int(sample_rate)
        self.frame = int(self.sample_rate * FRAME_MS / 1000)
        self.hop = int(self.sample_rate * HOP_MS / 1000)
        # Rolling analysis window: only the most recent ``frame`` samples are
        # ever read, so keep just those as a float32 array (same as v0).
        ...
        # The v6 layers draw from their own stream, so the gesture directions
        # stay the ones v5 would pick for the same audio and seed.
        self.rng_layers = np.random.default_rng(self._seed + 1)
    ...
    def _reset_state(self) -> None:
        ...
        self.target_dir: NDArray[np.float32] = np.zeros(6, dtype=np.float32)
        self.current_dir: NDArray[np.float32] = np.zeros(6, dtype=np.float32)
    ...
    def reset(self) -> None:
        """Reset the DSP state and buffers, keep the seed and phases."""
    ...
    def _new_target_direction(self, prominence: float) -> NDArray[np.float32]:
        raw = self.rng.uniform(-1.0, 1.0, size=6).astype(np.float32)
        d: NDArray[np.floating[Any]] = np.sign(raw) * np.abs(raw) ** 1.5
    ...
    def feed(self, pcm: NDArray[np.float32]) -> list[dict[str, float]]:
        """Consume PCM and return one offsets dict per completed hop."""
        if pcm.size == 0:
            return []
        if self.carry.size:
        ...
        while self.carry.size >= self.hop:
            ...
            # Phrase-level layers.
            if phrase_start:
                self.drift_target_deg = float(self.rng_layers.uniform(-1.0, 1.0)) * prof.drift_yaw_deg
    ...
    def _on_nucleus(self, prominence: float) -> None:
        ...
        if (
            ...
            and float(self.rng_layers.random()) < prof.drop_prob
        ):
            self.drop_phase = 0
            self.last_drop_age = 0

    def _maybe_start_final_tilt(self) -> None:
        """Decide the phrase-final gesture. Called when the gate closes."""
        prof = self.profile
