...
async def update_reachy_mini(
    ...
    cmd, extra_env = build_install_command(
        extras="wireless-version",  #TODO: replace "wireless-version" with "wireless-version,gstreamer" when the wheel is released
        git_ref=git_ref, 
    ...
    if apps_venv_python.exists():
        ...
        cmd, extra_env = build_install_command( 
            extras="",  #TODO: replace "" with "gstreamer" when the wheel is released
            git_ref=git_ref, 
            pre_release=pre_release,
