#!/usr/bin/env python3
# reachy_rhythm_controller.py  – v10.2 (Final, Corrected)
"""
Real-time robot rhythm synchronization using a continuous, incremental clock.

This script synchronizes a Reachy Mini robot's movements to music by advancing a
continuous "musical time" on each frame. This ensures the robot's phase
is always smooth, even when the detected BPM changes.

- A phase offset is calculated to keep the robot's timing aligned with
  detected real-world beats by applying small, continuous corrections.
- This "smart correction" can be toggled with the 'P' key.
- Manual phase offset can be adjusted with Left/Right arrow keys.
- A comprehensive, zoomable plot is generated on Ctrl-C for analysis.
"""
...
class Config:
    ...
    offset_correction_rate: float = 0.05
    phase_correction_limit: float = 0.01
...
class SharedState:
    ...
    def toggle_smart_offset(self) -> bool:
        with self.lock:
            ...
            return self.smart_offset_enabled
...
def audio_thread(state: MusicState, config: Config, stop_event: threading.Event) -> None:
    pa=pyaudio.PyAudio()
    stream=pa.open(format=pyaudio.paFloat32,channels=1,rate=config.audio_rate,input=True,frames_per_buffer=config.audio_chunk_size)
...
def generate_final_plot(log, offset_log, config):
    ...
    final_dance_time = np.array([e['final_dance_time'] for e in log])
    acc_beats = np.array([b for e in log for b in e['accepted_beats']]) - start_time
    
    ax[0].plot(t, active_bpm, '-', label='Active BPM (from Librosa)')
    ax[0].set_ylabel('BPM'); ax[0].set_ylim(0, 200); ax[0].legend(); ax[0].grid(True, alpha=0.3)
    
    ax[1].plot(t, np.sin(2 * np.pi * final_dance_time), '-', label='Final Robot Phase (sin)')
    ax[1].plot(t, phase_error, '-', label='Calculated Phase Error', alpha=0.7)
    ...
    print(f"Analysis plot saved to {path}")
    plt.show()
...
def main(config: Config) -> None:
    ...
    musical_time, phase_error = 0.0, 0.0
        ...
        try:
            while True:
                ...
                if manual_offset != last_manual_offset:
                    offset_change_log.append((time.time(), manual_offset))
                    last_manual_offset = manual_offset
                
                final_dance_time = musical_time
                if active_bpm > 0:
                    musical_time += dt * (active_bpm / 60.0)
                    if smart_offset_on:
                        ...
                        # limit correction to avoid overshooting
                        correction = np.clip(correction, -config.phase_correction_limit, config.phase_correction_limit)
                        musical_time += correction
                    else:
                        phase_error = 0.0
                    
                    final_dance_time = musical_time + manual_offset
                    
                    offs = move_fn(final_dance_time, **params)
                ...
                full_log.append({'t':time.time(),'active_bpm':active_bpm,'phase_error':phase_error,'final_dance_time':final_dance_time,'accepted_beats':accepted_this_frame})
                
                # FIXED: Use the correct start time of the loop for sleep calculation.
