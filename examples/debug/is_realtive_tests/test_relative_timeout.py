...
def main():
    ...
    with ReachyMini() as mini:
        ...
        while time.time() - start_time < 5.0:
            ...
            print(f"\rActive motion: {elapsed_time:.1f}/5.0s, pitch={math.degrees(pitch_offset_rad):.1f}°", end="")
        ...
        while time.time() - resume_time < 3.0:
            ...
            x_offset_m = 0.02 * math.sin(2 * math.pi * 1.0 * elapsed_time)  # 2cm amplitude, 1Hz
            z_offset_m = 0.01 * math.sin(2 * math.pi * 1.3 * elapsed_time)  # 1cm amplitude, 1.3Hz 
            roll_offset_rad = math.radians(15.0) * math.sin(2 * math.pi * 0.7 * elapsed_time)  # 15° amplitude, 0.7Hz
            
            relative_pose = utils.create_head_pose(
                x_offset_m, 0, z_offset_m,  # X and Z translation
                roll_offset_rad, 0, 0,  # Roll rotation only
                degrees=False
            )
            ...
            print(f"\rLarge motion: {elapsed_time:.1f}/3.0s | X={x_offset_m*1000:.1f}mm, Z={z_offset_m*1000:.1f}mm, Roll={math.degrees(roll_offset_rad):.1f}°", end="")
            time.sleep(control_period)
        ...
        while time.time() - stop_time2 < observation_duration2:
            ...
            print(f"\rObserving large motion decay: {elapsed_since_stop:.1f}/4.0s", end="")
            time.sleep(0.1)  # Slower update rate for observation
