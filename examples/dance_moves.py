...
def move_simple_nod(t_beats, amplitude_rad, antenna_move_name='wiggle', **kwargs):
    ...
    antennas = AVAILABLE_ANTENNA_MOVES[antenna_move_name](t_beats, **get_antenna_kwargs(kwargs))
...
def move_headbanger_combo(t_beats, pitch_amplitude_rad, z_amplitude_m, antenna_move_name='both', **kwargs):
    ...
    antenna_kwargs = get_antenna_kwargs(kwargs); antenna_kwargs['amplitude'] = pitch_amplitude_rad * 2.0
...
def move_neck_recoil(t_beats, amplitude_m, antenna_move_name='wiggle', **kwargs): # FIXED
...
def move_side_glance_flick(t_beats, yaw_amplitude_rad, antenna_move_name='wiggle', **kwargs): # FIXED
    period = 4.0; t_in_period = t_beats % (1.0/kwargs.get('cycles_per_beat', 0.25))
    def ease(t): return t*t*(3-2*t)
    yaw_offset = 0
    if t_in_period < 0.5: yaw_offset = yaw_amplitude_rad * ease(t_in_period / 0.5)
    elif t_in_period < 1.5: yaw_offset = yaw_amplitude_rad
    else: yaw_offset = yaw_amplitude_rad * (1.0 - ease((t_in_period - 1.5) / (period-1.5)))
...
MOVE_SPECIFIC_PARAMS = {
    ...
    "dizzy_spin": {"roll_amplitude_rad": np.deg2rad(15), "pitch_amplitude_rad": np.deg2rad(15), "cycles_per_beat": 0.25, **DEFAULT_ANTENNA_PARAMS}, # TUNED
    ...
    "interwoven_spirals": {"roll_amp": np.deg2rad(15), "pitch_amp": np.deg2rad(20), "yaw_amp": np.deg2rad(25), "cycles_per_beat": 0.125, **DEFAULT_ANTENNA_PARAMS}, # TUNED
