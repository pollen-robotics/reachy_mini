...
def parse_args():
    parser = argparse.ArgumentParser(description="Script for training and testing with data folders.")
...
def main():
    ...
    train_data = train_data['file_name'].progress_apply(get_mfcc, sample_rate=args.sample_rate)
    test_data = test_data['file_name'].progress_apply(get_mfcc, sample_rate=args.sample_rate)

    train_data['file_name'] = audio_train_files
    test_data['file_name'] = audio_test_files

    train_data['label'] = train_data['file_name'].progress_apply(lambda f : f.stem.split('_')[0])
    test_data['label'] = test_data['file_name'].progress_apply(lambda f : f.stem.split('_')[0])
    ...
    # Fit an SVM model
    train_features_split, test_features_split, train_index_split, test_index_split = train_test_split(train_features_pca, train_index, test_size = 0.2, random_state = 42, shuffle = True)

    clf = SVC(kernel = 'rbf', probability=True)

    clf.fit(train_features_split, train_index_split)

    print("Accuracy score :", accuracy_score(clf.predict(test_features_split), test_index_split))
