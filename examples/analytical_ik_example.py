...
def main():
    with ReachyMini() as mini:
        ...
        for i, label in enumerate(labels):
            ...
            tk.Scale(root, variable=var, from_=-0.05 if i < 3 else -1.57, to=0.05 if i < 3 else 1.57, resolution=0.001, orient="horizontal", length=200).grid(row=i, column=1)
        ...
        while True:
            ...
            T_world_target = analytic_solver.T_world_head_home @ tf.translation_matrix((px, py, pz)) @ tf.euler_matrix(roll, pitch, yaw)
