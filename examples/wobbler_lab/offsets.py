"""Export head offsets from a wobbler version to the theater player's JSON.
...
JSON format (see agentic_robot_theater/scenes/couple_fight/FORMATS.md)::
...
def load_scene_beats(scene_path: Path | None) -> dict[str, dict]:
...
def colouring_for(version: str, beat: dict | None, emotion: str | None, energy: float | None) -> tuple[str, float]:
...
def export(wav: Path, version: str, emotion: str, energy: float, out: Path) -> dict:
