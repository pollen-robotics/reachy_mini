...
Run from the repo root::
    ...
    python examples/wobbler_lab/compare.py \
        --wav-dir path/to/scene/audio --scene path/to/scene/scene.json \
        --versions v0,v5,v6 --out-dir examples/wobbler_lab/out/my_scene
...
from wobbler_lab.offsets import colouring_for, load_scene_beats  # noqa: E402
...
@dataclass
class LineResult:
    """Metrics of one version on one line of the scene."""
...
def plot_line(
    ...
    """Write the stacked plot of one line: audio rows, then two rows per version."""
    versions = list(sims)
...
def plot_summary(rows: list[LineResult], versions: list[str], out_path: Path) -> None:
    """Write the per-line, per-version metrics panels to *out_path*."""
...
def write_metrics_md(rows: list[LineResult], versions: list[str], out_path: Path) -> None:
    """Write the metrics as a Markdown table to *out_path*."""
    lines = [
        "# Wobbler comparison: " + " / ".join(versions),
        "",
...
def main() -> None:
    """Compare the versions on every wav of a directory."""
