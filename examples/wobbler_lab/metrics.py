...
@dataclass
class SyncMetrics:
    """Three scores of how well a motion track follows the speech."""
...
def motion_magnitude(motion: NDArray[np.float32]) -> NDArray[np.float32]:
    ...
    magnitude: NDArray[np.float32] = np.sum(np.abs(motion / norm), axis=1).astype(np.float32)
    return magnitude


def motion_velocity(motion: NDArray[np.float32]) -> NDArray[np.float32]:
    """Per-axis-normalized |motion difference|."""
    if len(motion) < 2:
        return np.zeros(len(motion), dtype=np.float32)
    diff = np.diff(motion, axis=0, prepend=motion[:1])
    norm = np.std(diff, axis=0) + 1e-9
    velocity: NDArray[np.float32] = np.sum(np.abs(diff / norm), axis=1).astype(np.float32)
    return velocity
