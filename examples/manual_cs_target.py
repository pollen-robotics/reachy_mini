...
def main():
    with ReachyMini() as mini:
        ...
        tk.Scale(root, variable=roll_var, from_=-90, to=90, orient=tk.HORIZONTAL).grid(row=0, column=1)
        tk.Label(root, text="Pitch (deg):").grid(row=1, column=0)
        tk.Scale(root, variable=pitch_var, from_=-90, to=90, orient=tk.HORIZONTAL).grid(row=1, column=1)
        tk.Label(root, text="Yaw (deg):").grid(row=2, column=0)
        tk.Scale(root, variable=yaw_var, from_=-90, to=90, orient=tk.HORIZONTAL).grid(row=2, column=1)
        ...
        tk.Scale(root, variable=x_var, from_=-0.05, to=0.05, resolution=0.001, orient=tk.HORIZONTAL).grid(row=3, column=1)
        tk.Label(root, text="Y (m):").grid(row=4, column=0)
        tk.Scale(root, variable=y_var, from_=-0.05, to=0.05, resolution=0.001, orient=tk.HORIZONTAL).grid(row=4, column=1)
        tk.Label(root, text="Z (m):").grid(row=5, column=0)
        tk.Scale(root, variable=z_var, from_=-0.05, to=0.05, resolution=0.001, orient=tk.HORIZONTAL).grid(row=5, column=1)
        
        # add a checkbox to enable/disable collision checking
        collision_check_var = tk.BooleanVar(value=False)
        tk.Checkbutton(root, text="Check Collision", variable=collision_check_var).grid(row=6, column=1)
        ...
        while True:
            ...
            mini.set_position(head=head, antennas=np.array([target, -target]), check_collision= collision_check_var.get())
