...
class SharedState:
    ...
    def toggle_pause(self):
        with self.lock: self.running = not self.running
        return self.running
    ...
    def get_and_clear_changes(self) -> dict:
        with self.lock:
            changes = {"next_move":self.next_move,"prev_move":self.prev_move,"next_waveform":self.next_waveform,"bpm_change":self.bpm_change,"amplitude_change":self.amplitude_change}
...
def keyboard_listener_thread(shared_state: SharedState, stop_event: threading.Event):
    def on_press(key):
        ...
        if hasattr(key,'char'):
            if key.char.lower()=='q': stop_event.set(); return False
            if key.char.lower()=='p': shared_state.toggle_pause()
            if key.char.lower()=='w': shared_state.trigger_next_waveform()
            if key.char=='+': shared_state.adjust_amplitude(0.1)
            if key.char=='-': shared_state.adjust_amplitude(-0.1)
        if key==keyboard.Key.space: shared_state.toggle_pause()
        elif key==keyboard.Key.right: shared_state.trigger_next_move()
        elif key==keyboard.Key.left: shared_state.trigger_prev_move()
        elif key==keyboard.Key.up: shared_state.adjust_bpm(5.0)
        elif key==keyboard.Key.down: shared_state.adjust_bpm(-5.0)
...
def main(config: Config):
    ...
    try: current_move_idx = move_names.index(config.start_move)
    except ValueError:
        print(f"Warning: Start move '{config.start_move}' not found. Starting with the first move."); current_move_idx = 0
    ...
    try:
        with ReachyMini() as mini: # Changed from 'bot' to 'mini'
            ...
            while not stop_event.is_set():
                ...
                if changes["bpm_change"]: bpm = max(20.0, bpm + changes["bpm_change"])
                if changes["amplitude_change"]: amplitude_scale = max(0.1, amplitude_scale + changes["amplitude_change"])
                if changes["next_move"]: current_move_idx = (current_move_idx + 1) % len(move_names); sequence_beat_counter = 0
                if changes["prev_move"]: current_move_idx = (current_move_idx - 1 + len(move_names)) % len(move_names); sequence_beat_counter = 0
                if changes["next_waveform"]: current_waveform_idx = (current_waveform_idx + 1) % len(waveforms)
                
                if not shared_state.running:
                    mini.set_target(head_pose(config.neutral_pos, config.neutral_eul), antennas=np.zeros(2)) # Changed to 'mini'
                ...
                if 'waveform' in move_fn.__code__.co_varnames: current_params['waveform'] = waveform
                for key in current_params:
                    if 'amplitude' in key: current_params[key] *= amplitude_scale

                offsets = move_fn(t_beats, **current_params)
                
                final_pos, final_eul, final_ant = config.neutral_pos + offsets.position_offset, config.neutral_eul + offsets.orientation_offset, offsets.antennas_offset
                mini.set_target(head_pose(final_pos, final_eul), antennas=final_ant) # Changed to 'mini'
                ...
                if loop_start_time - last_status_print_time > 1.0:
                    ...
                    print(f"[{status}] Move: {move_name:<35} | BPM: {bpm:<5.1f} | Wave: {wave_status:<8} | Amp: {amplitude_scale:.1f}x", end="")
                    sys.stdout.flush()
                ...
                if loop_start_time - last_orb_update_time > 4.0:
                    line_to_display = ORB_HELP_LINES[current_orb_line_idx]
                    try: mini.orb.display_text(line_to_display, wait=False)
                    except Exception as e: pass # Don't crash if orb fails
                    current_orb_line_idx = (current_orb_line_idx + 1) % len(ORB_HELP_LINES)
        ...
        try:
            with ReachyMini(force_connect=True) as mini: # Changed to 'mini'
                mini.orb.display_text("") # Clear the orb display
                mini.go_to_sleep()
        except Exception: print("Could not connect to robot for final sleep command.")
        print("Shutdown complete.")


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Interactive Dance Move Tester for Reachy Mini.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    app_config = Config(bpm=cli_args.bpm, start_move=cli_args.start_move, beats_per_sequence=cli_args.beats_per_sequence)
    main(app_config)