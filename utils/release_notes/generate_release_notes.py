...
def check_opencode_model(model: str) -> None:
    ...
    if model not in available:
        raise RuntimeError(
            ...
            f"Expected the full `provider/model` form (e.g. `huggingface/zai-org/GLM-5.2`). "
...
def run_opencode_skill(
    ...
    # Run opencode non-interactively. --auto approves tool/skill permissions, which is
    # required in CI (no TTY, so prompts would otherwise be auto-rejected).
    cmd = [opencode_cmd, "run", "--auto"]
...
def main(since_tag: str, bump_type: str = "patch", max_iterations: int = 3, seed: str | None = None) -> int:
    ...
    for i in range(max_iterations):
        ...
        if i < max_iterations - 1:
            ...
            if not run_opencode_skill("reachy-mini-release-notes:validate", version, missing or None, extra or None):
...
def cli():
    ...
    parser = argparse.ArgumentParser(
        description="Generate release notes for reachy_mini",
