...
def _make(
    ...
    # Mode policy only; the settle gate is tested in test_head_tracking_backend.
    backend._tracking_settle_rad = float("inf")
...
def _publish_and_step(
    ...
    # The yaw round-trips through a rotation matrix; round off the 1e-17 noise.
    return None if rpy is None else round(rpy[2], 12)
