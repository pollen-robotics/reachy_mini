...
The robot lock is the daemon's single source of truth for *"is this
robot in use?"*. It serializes two independent code paths that can
drive the robot: local Python apps launched by :class:`AppManager`
and remote WebRTC clients routed through the central signaling relay.

### Concurrency model

Two independent entry points can open a session with the robot:
...
and fight over motor commands, camera, and audio. :class:`RobotLock`
...
- The local path uses :meth:`RobotLock.acquire_local_evicting_remote`.
...
- The remote path uses :meth:`RobotLock.try_acquire_remote`. This
...
- :meth:`RobotLock.release_local` is called from the subprocess
  monitor's ``finally`` block, so clean exits, crashes, ``SIGKILL``,
  OOM and task cancellation all release the lock.
- :meth:`RobotLock.release_remote` is called from every
...
[[autodoc]] reachy_mini.daemon.robot_lock.RobotLock

[[autodoc]] reachy_mini.daemon.robot_lock.RobotLockState

[[autodoc]] reachy_mini.daemon.robot_lock.RobotLockStatus
...
[[autodoc]] reachy_mini.daemon.app.routers.daemon.get_robot_lock_status
