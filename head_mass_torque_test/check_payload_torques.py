#!/usr/bin/env python3
"""
Check Motor Torques with Variable Payload Mass

Tests different payload masses in 50g increments until reaching safety torque limit.
Uses reachy_mini_payload.xml and modifies fake_payload_head mass.
"""
...
# Payload mass testing - start from 50g and increment
PAYLOAD_START_MASS_KG = 0.050  # 50g starting payload
MASS_INCREMENT_KG = 0.05  # 50g increments
MAX_MASS_KG = 2.0  # Maximum payload to test

# XML path - use the payload version
SCRIPT_DIR = Path(__file__).parent
REPO_ROOT = SCRIPT_DIR.parent
XML_PATH = REPO_ROOT / "src/reachy_mini/descriptions/reachy_mini/mjcf/reachy_mini_payload.xml"
...
def modify_payload_mass_in_model(model, new_mass_kg):
    """
    Modify the fake_payload_head body mass directly in the MuJoCo model.
    ...
    # Set new mass (without changing COM since inertial pos is 0 0 0)
    model.body_mass[body_id] = new_mass_kg

    return original_mass


def get_original_payload_mass():
    """Get the original payload mass from the XML file."""
    tree = ET.parse(XML_PATH)
    root = tree.getroot()

    for body in root.iter("body"):
        if body.get("name") == "fake_payload_head":
            inertial = body.find("inertial")
            if inertial is not None:
                return float(inertial.get("mass"))

    raise ValueError("Could not find fake_payload_head mass in XML")
...
def plot_torques(result, output_path):
    ...
    payload_g = result.get("payload_mass_g", "unknown")
    ax.set_title(
        f"Motor Torques - {result['pose_name']} (Payload: {payload_g}g)\n"
...
def create_summary_figure(all_mass_results, output_path):
    """Create summary figure showing torque vs payload mass."""
    ...
    for result in all_mass_results:
        ...
        poses_data[pose_key]["masses"].append(result["payload_mass_g"])
    ...
    ax1.set_xlabel("Payload Mass (g)", fontsize=12, fontweight="bold")
    ax1.set_ylabel("Max Torque (N·m)", fontsize=12, fontweight="bold")
    ax1.set_title("Maximum Torque vs Payload Mass", fontsize=14, fontweight="bold")
    ax1.grid(alpha=0.3)
    ...
    # Get mass range
    all_masses = [r["payload_mass_g"] for r in all_mass_results]
    min_mass = min(all_masses)
    max_tested_mass = max(all_masses)

    # Build statistics text
    stats_text = "PAYLOAD EXPERIMENT STATISTICS\n"
    stats_text += "=" * 50 + "\n\n"

    stats_text += f"Starting payload mass: {min_mass}g\n"
    ...
    stats_text += f"Payload:  {max_result['payload_mass_g']}g\n"
    ...
    stats_text += "-" * 50 + "\n"
    stats_text += "• Testing payload mass on top of head\n"
    stats_text += "• Torque increases with payload mass\n"
...
def test_pose(backend, kinematics, pose_dict, pose_key, payload_mass_g, output_dir=None):
    """
    ...
    modify_payload_mass_in_model(backend.model, payload_mass_g / 1000.0)
    ...
    if output_dir:
        ...
        # Save robot render
        render_path = output_dir / f"{pose_key}_{payload_mass_g}g_payload_render.png"
        render_robot_image(backend, render_path)
        result["render_path"] = str(render_path)

        # Save torque graph
        graph_path = output_dir / f"{pose_key}_{payload_mass_g}g_payload_torques.png"
...
def test_mass(mass_kg, output_dir):
    """
    Test all poses with a specific payload mass.
    ...
    print(f"\n{'='*80}")
    print(f"TESTING PAYLOAD MASS: {mass_g}g ({mass_kg:.3f} kg)")
    print(f"{'='*80}")

    # Initialize backend with payload XML
    ...
    original_mass = modify_payload_mass_in_model(backend.model, mass_kg)

    if abs(mass_kg - original_mass) < 0.001:
        print(f"Using original payload mass: {original_mass:.4f} kg")
    else:
        print(f"Modified payload mass from {original_mass:.4f} kg to {mass_kg:.4f} kg")
...
def main():
    print("=" * 80)
    print("HEAD PAYLOAD TORQUE CHECK - VARIABLE PAYLOAD MASS")
    print("=" * 80)
    print(f"\nMotor stall torque: {STALL_TORQUE_NM} N·m")
    print(f"Safe limit (75%): {SAFE_LIMIT_NM} N·m")
    print(f"Test poses: {len(TEST_POSES)}")
    print(f"Starting payload mass: {int(PAYLOAD_START_MASS_KG * 1000)}g")
    ...
    if not XML_PATH.exists():
        ...
        print("Please make sure reachy_mini_payload.xml exists")
        return

    # Create output directory
    output_dir = Path("payload_torque_check_results")
    output_dir.mkdir(exist_ok=True)
    print(f"Output directory: {output_dir}\n")

    # Get original payload mass from XML
    original_mass_kg = get_original_payload_mass()
    print(
        f"Original payload mass from XML: {original_mass_kg:.4f} kg ({int(original_mass_kg * 1000)}g)\n"
    )

    # Test incrementally starting from PAYLOAD_START_MASS_KG
    all_results = []
    current_mass = PAYLOAD_START_MASS_KG
    limit_reached = False
    ...
    while current_mass <= MAX_MASS_KG and not limit_reached:
        ...
        if reached:
            limit_reached = True
            print(f"\n⚠ Torque limit reached at payload {int(current_mass * 1000)}g!")
            max_safe_mass = current_mass - MASS_INCREMENT_KG
            print(f"✓ Max safe payload mass: {int(max_safe_mass * 1000)}g")
            break

        current_mass += MASS_INCREMENT_KG

    # Save results to file
    results_file = output_dir / "payload_test_results.npy"
    ...
    if all_results:
        ...
        summary_path = output_dir / "payload_test_summary.png"
    ...
    if limit_reached:
        ...
        if limit_results:
            ...
            print(f"\nLimit reached at payload: {hardest['payload_mass_g']}g")
        ...
        print(f"At payload: {max_result['payload_mass_g']}g, pose: {max_result['pose_name']}")

    print(f"\n✓ All results saved to: {output_dir}/")
    print(f"  - Individual renders and graphs for each payload/pose combination")
    print(f"  - Summary: payload_test_summary.png")
    print("=" * 80)
